...
class WechatService
{
    ...
    {
        $wechat = SystemConfigService::more(['wechat_appid', 'wecaht_app_appid', 'wechat_app_appsecret', 'wechat_appsecret', 'wechat_token', 'wechat_encodingaeskey', 'wechat_encode']);
        ...
        if (request()->isApp()) {
            $appId = isset($wechat['wecaht_app_appid']) ? trim($wechat['wecaht_app_appid']) : '';
    ...
    private static function hook($server)
    {
        ...
        $server->setMessageHandler(function ($message) use ($messageService, $wechatReplyService) {
            switch ($message->MsgType) {
                case 'event':
                    switch (strtolower($message->Event)) {
                        case 'subscribe':
                            [$response, $spid] = $messageService->wechatEventSubscribe($message);
                            break;
                        case 'unsubscribe':
                            $messageService->wechatEventUnsubscribe($message);
                            break;
                        case 'scan':
                            [$response, $spid] = $messageService->wechatEventScan($message);
                            break;
                        case 'location':
            ...
            $wechatMessage->wechatMessageBefore($message, $spid);
...
//        ])->getBody()->getContents());
     ...
     * @return Order
     */
    protected static function paymentOrder($openid, $out_trade_no, $total_fee, $attach, $body, $detail = '', $trade_type = Order::JSAPI, $options = [])
    {
     ...
     * @return mixed
     */
    public static function paymentPrepare($openid, $out_trade_no, $total_fee, $attach, $body, $detail = '', $trade_type = Order::JSAPI, $options = [])
    {
