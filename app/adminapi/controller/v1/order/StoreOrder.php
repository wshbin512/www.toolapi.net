...
class StoreOrder extends AuthController
{
    ...
    public function lst()
    {
        ...
        return $this->success($this->services->getOrderList($where, ['*'], ['pink', 'invoice']));
    }
    ...
    public function del_orders()
    {
        ...
        if (!count($ids) && $all == 0) return $this->fail('请选择需要删除的订单');
        if ($this->services->getOrderIdsCount($ids) && $all == 0) return $this->fail('您选择的的订单存在用户未删除的订单');
        if ($all == 0 && $this->services->batchUpdate($ids, ['is_system_del' => 1])) return $this->success('删除成功');
        if ($all == 1) $ids = [];
        $type = 6;// 订单删除
        $where['status'] = -4;
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueService->dataAddRedis($where, 'id', $ids, $type);
        Queue::instance()->job(\crmeb\jobs\BatchHandleJob::class)->data(false, $type)->setQueueName("BATCH-CRMEB")->push();
        return $this->success('后台程序已执行批量删除任务!');
    ...
    public function update_refund_integral(StoreOrderRefundServices $services, $id)
    {
        ...
        if ($bj < 0) {
            return $this->fail('退积分大于支付积分，请修改退积分');
        }
        //积分退款处理
    ...
    public function hand_batch_delivery()
    {
        ...
        if (!$data['file']) return $this->fail('请上传文件');
        $file = public_path() . substr($data['file'], 1);
        $type = 7;//手动批量发货
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $expreData = $this->services->readExpreExcel($file, 4);
        $queueId = $queueService->dataAddRedis([], false, $expreData, $type);
        $data['queueType'] = $type;
        $data['cacheType'] = 3;
        $data['type'] = 1;
        $data['queueId'] = $queueId ? $queueId : 0;
        $this->services->adminQueueOrderDo($data);
        return $this->success('后台程序已执行批量发货任务!');
    ...
    public function other_batch_delivery()
    {
        ...
        if ($data['all'] == 0 && empty($data['ids'])) return $this->fail('请选择需要发货的订单');
        if ($data['express_record_type'] == 2 && !sys_config('config_export_open', 0)) return $this->fail('请先在系统设置中打开单子面单打印开关');
        ...
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueId = $queueService->dataAddRedis($data['where'], 'id', $data['ids'], $data['queueType']);
        $data['queueId'] = $queueId ? $queueId : 0;
