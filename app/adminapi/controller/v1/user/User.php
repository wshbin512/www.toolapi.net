...
class User extends AuthController
{
    ...
    public function save_give_level_time($id)
    {
        ...
        return $this->success($this->services->saveGiveLevelTime((int)$id, (int)$days) ? '赠送成功' : '赠送失败');
    }
    ...
    {
        list($group_id, $uids, $all, $where) = $this->request->postMore([
            ['group_id', 0],
            ['uids', ''],
            ['all', 0],
            ['where', ""],
        ...
        $queueService->dataAddRedis($where, 'uid', $uids, $type, $group_id);
        Queue::instance()->job(\crmeb\jobs\BatchHandleJob::class)->data($group_id, $type)->setQueueName("BATCH-CRMEB")->push();
        return $this->success('后台程序已执行用户分组任务!');
    }

    public function save_set_group1()
    {
    ...
    public function save_set_label()
    {
        ...
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueService->dataAddRedis($where, 'uid', $uids, $type, $lables);
        Queue::instance()->job(\crmeb\jobs\BatchHandleJob::class)->data($lables, $type)->setQueueName("BATCH-CRMEB")->push();
     ...
     * @return mixed
     */
    public function save_set_label1()
    {
