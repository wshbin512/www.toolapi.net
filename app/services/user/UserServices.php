...
class UserServices extends BaseServices
{
    ...
    public function getUserRegister(array $where){
    ...
    public function setUserInfo($user, int $spreadUid = 0, string $userType = 'wechat')
    {
        ...
        if ($spreadUid) {
            ...
            $storeCoupon->userFirstSubGiveCoupon((int)$res->uid);
            //绑定成功发送消息
     ...
     * @throws Exception
     */
    public function useNumber(int $uid)
    {
        $userInfo = $this->getUserInfo($uid);
        if (!$userInfo) {
            throw new ValidateException('数据不存在');
        }
        $use_number = $userInfo['use_number'] + 1;
        if (!$this->dao->update($uid, ['use_number' => $use_number]))
            throw new Exception('增加用户使用次数成功');
        return true;
    }
    ...
    {
        if (!$this->dao->update($uid, ['now_money' => bcadd($brokerage_price, $price, 2)]))
     ...
     * @param int $quicken
     * @param int $old_red
     * @param int $old_quicken
     * @return bool
     * @throws Exception
     */
    public function addRedGreen(int $uid, int $red, int $quicken, int $old_red, int $old_quicken)
    {
        if (!$this->dao->update($uid, ['coupon_red' => bcadd($old_red, $red, 2),'quicken' => bcadd($old_quicken, $quicken, 2)]))
            throw new Exception('增加用户红券失败');
        return true;
    }
     ...
     * 增加用户购物卡
     * @param int $uid
     * @param float $old_shopping
     * @param float $shopping
     * @return bool
     * @throws Exception
     */
    public function addShopping(int $uid, $old_shopping, $shopping)
    {
        if (!$this->dao->update($uid, ['shopping_card' => bcadd($old_shopping, $shopping, 2)]))
            throw new Exception('增加用户购物卡失败');
        return true;
    }
     ...
     * @throws Exception
     */
    public function cutShopping(int $uid, $old_shopping, $shopping)
    {
        if (!$this->dao->update($uid, ['shopping_card' => bcsub($old_shopping, $shopping, 2)]))
            throw new Exception('减少用户购物卡失败');
        return true;
    }
    ...
    public function index(array $where)
    {
        ...
        if ($list) {
            ...
            $spread_names = $this->dao->getColumn([], 'nickname', 'uid');
    ...
    public function edit(int $id)
    {
        ...
        if ($storeBrokerageStatus == 1) {
            $f[] = Form::radio('is_promoter', '群主权限', $user->getData('is_promoter'))->info('指定分销模式下，开启或关闭群主的推广权限')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        }
        $f[] = Form::radio('is_head', '团长权限', $user->getData('is_head'))->info('指定分销模式下，开启或关闭团长的发起拼购权限')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        $f[] = Form::radio('is_community', '区域权限', $user->getData('is_community'))->info('区域管理员')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        $f[] = Form::radio('is_company', '分公司权限', $user->getData('is_company'))->info('分公司管理员')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
    ...
    public function saveForm()
    {
        ...
        $f[] = Form::radio('spread_open', '推广资格', 1)->info('禁用用户的推广资格后，在任何分销模式下该群主都无分销权限')->options([['value' => 1, 'label' => '启用'], ['value' => 0, 'label' => '禁用']]);
        //分销模式  人人分销
        $storeBrokerageStatus = sys_config('store_brokerage_statu', 1);
        if ($storeBrokerageStatus == 1) {
            $f[] = Form::radio('is_promoter', '群主权限', 0)->info('指定分销模式下，开启或关闭群主的推广权限')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        }
        $f[] = Form::radio('is_head', '团长权限', 1)->info('指定分销模式下，开启或关闭团长的发起拼购权限')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        $f[] = Form::radio('is_community', '区域权限', 1)->info('区域管理员')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        $f[] = Form::radio('is_company', '分公司权限', 1)->info('分公司管理员')->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
    ...
    public function updateInfo(int $id, array $data)
    {
        ...
        if ($data['money_status'] && $data['money']) {//余额增加或者减少
            $bill_data = ['link_id' => $data['adminId'] ?? 0, 'number' => $data['money']];
            if ($data['money_status'] == 1) {//增加
                ...
                $bill_data['balance'] = $edit['now_money'];
        ...
        if (!isset($data['is_other']) || !$data['is_other']) {
            ...
            $edit['mark'] = $data['mark'];
            $edit['is_head'] = $data['is_head'];
            $edit['is_promoter'] = $data['is_promoter'];
            $edit['is_community'] = $data['is_community'];
            $edit['is_company'] = $data['is_company'];
     ...
     * @return mixed
     */
    public function setLabel($uids, $all, $where)
    {
    ...
    public function giveLevelTime($id)
    {
        ...
        $field[] = Form::input('days', '增加时长(天)')->maxlength(6);
    ...
    public function personalHome(array $user, $tokenData)
    {
        ...
        $user['service_num'] = $servicesRecord->sum(['user_id' => $uid], 'mssage_num');
        return $user;
    }
    ...
    public function getUserSpreadGrade(int $uid = 0, $grade = 0, $orderBy = '', $keyword = '')
    {
        ...
        if ($grade == 0) {
            ...
            $list = $userStoreOrder->getUserSpreadCountList($userStair, $orderBy, $keyword);
        } else {
            $userSecondary = $this->dao->getColumn([['spread_uid', 'IN', $userStair]], 'uid');
            $data['total'] = count($userStair);
            $data['totalLevel'] = count($userSecondary);
            $list = $userStoreOrder->getUserSpreadCountList($userSecondary, $orderBy, $keyword);
        }
        foreach ($list as &$item) {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function offMemberLevel($uid)
    {
        if (!$uid) return false;
        $user_info = $this->dao->get($uid);
        if (!$user_info) return false;
        if ($user_info['is_ever_level'] == 0 && $user_info['is_money_level'] > 0 && $user_info['overdue_time'] < time()) {
    ...
    /**查询用户数据，并将数据放入redis有序集合中
     * @param array $where
     * @param string $field
     * @return mixed
     */
    public function userAddRedis(array $where = [], $field = "*", array $uid = [], string $redisKey, $type)
    {
        if (!$redisKey) throw new ValidateException('缺少集合缓存key');
        if (!$type) throw new ValidateException('缺少队列执行类型');
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueService->isAddQueueRedisCheck($redisKey, $type);
        return $queueService->getQueueRedisData($redisKey, $type, $uid, $where, $field);
    }

    /**后台发送优惠券
     * @param $coupon
     */
    public function adminSendCoupon($coupon, $type)
    {
        ...
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueType = $queueService->getQueueOne(['type' => $type, 'status' => [0, 3]]);
        if (!$queueType) throw new ValidateException('前往任务列表清除异常数据，再手动执行');
        //把队列需要执行的入参数据存起来，以便队列执行失败后接着执行，同时队列状态改为正在执行状态。
        $queueService->addQueueDoing($coupon, $queueType['id']);
        $redisKey = $queueType['execute_key'];
        $uids = $redis->sMembers($redisKey);
        $chunkUids = array_chunk($uids, 1000, true);
        foreach ($chunkUids as $v) {
            $issueService->setCoupon($coupon, $v, $redisKey, $queueType);
        }
        //发完后将队列置为完成
        $queueService->addQueueSuccess($queueType['id'], $queueType['type'], []);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function adminUserGroup($groupId, $type)
    {
        if (!$type) return false;
        /** @var CacheService $redis */
        $redis = app()->make(CacheService::class);
        ...
        if (!$queueType) throw new ValidateException('前往任务列表清除异常数据，再手动执行');
        //把队列需要执行的入参数据存起来，以便队列执行失败后接着执行，同时队列状态改为正在执行状态。
        $queueService->addQueueDoing($groupId, $queueType['id']);
        $redisKey = $queueType['execute_key'];
        $uids = $redis->sMembers($redisKey);
        $chunkUids = array_chunk($uids, 1000, true);
        foreach ($chunkUids as $v) {
            $this->saveSetGroup($v, (int)$groupId, $redisKey, $queueType);
        }
        //发完后将队列置为完成
        $queueService->addQueueSuccess($queueType['id'], $queueType['type'], []);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function adminUserLabel($labelId, $type)
    {
        if (!$type) return false;
        /** @var CacheService $redis */
        $redis = app()->make(CacheService::class);
        ...
        if (!$queueType) throw new ValidateException('前往任务列表清除异常数据，再手动执行');
        //把队列需要执行的入参数据存起来，以便队列执行失败后接着执行，同时队列状态改为正在执行状态。
        $queueService->addQueueDoing($labelId, $queueType['id']);
        $redisKey = $queueType['execute_key'];
        $uids = $redis->sMembers($redisKey);
        $chunkUids = array_chunk($uids, 1000, true);
        foreach ($chunkUids as $v) {
            $this->saveBatchSetLabel($v, $labelId, $redisKey, $queueType);
        }
        //发完后将队列置为完成
        $queueService->addQueueSuccess($queueType['id'], $queueType['type'], []);
    }
