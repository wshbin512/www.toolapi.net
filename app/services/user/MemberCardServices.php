...
class MemberCardServices extends BaseServices
{
    ...
    public function getSearchList(array $where = [])
    {
        ...
        if ($where['phone']) {
            $uid = $userService->value(['phone' => $where['phone']], 'uid');
            if ($uid) $where['use_uid'] = $uid;
        }
    ...
    public function getExportData($where)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            if ($v['use_uid'] != 0) {
                ...
                $data[$k]['user_name'] = $userInfo['nickname'] ? $userInfo['nickname'] : $userInfo['account'];
     ...
     * @return bool
     */
    public function isOpenMemberCard($rightType = "")
    {
        /** @var SystemConfigService $systemConfigService */
        $systemConfigService = app()->make(SystemConfigService::class);
        /** @var MemberRightServices $memberRightService */
        $memberRightService = app()->make(MemberRightServices::class);
        $isOpen = $systemConfigService::more(['member_card_status']);
        //如果传入权益类别，查看是否具有某权益
        if (!$rightType) {
            if ($isOpen['member_card_status'] == 1) return true;
            return false;
        } else {
            $status = $memberRightService->getMemberRightStatus($rightType);
            if ($isOpen['member_card_status'] == 1 && $status) {
                $number = $memberRightService->getOne(['right_type' => $rightType]);
