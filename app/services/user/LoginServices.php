...
class LoginServices extends BaseServices
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function register($appId, $account = '', $password = '', $spread = 0, $user_type = 'h5', $address = '', $is_default = 0)
    {
        ...
        $data['card_id'] = '';
        $data['mark'] = '';
        $data['addres'] = $address ? $address['province'].$address['city'].$address['district'] : '';
        ...
        $data['city_id'] = $address ? $address['city_id'] : 0;
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function mobile($appId, $phone, $spread, $user_type = 'h5')
    {
        //数据库查询
        $user = $this->dao->getOne(['phone' => $phone]);
        if (!$user) {
            $user = $this->register($appId, $phone, '987654', $spread, $user_type);
            if (!$user) {
