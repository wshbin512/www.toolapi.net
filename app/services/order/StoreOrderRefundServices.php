...
//        $f[] = Form::radio('type', '状态', 1)->options([['label' => '直接退款', 'value' => 1], ['label' => '退款（无需退货，并且返回原状态）', 'value' => 2]]);
    ...
    public function payOrderRefund(int $type, $order, array $refundData)
    {
        return $this->transaction(function () use ($type, $order, $refundData) {
            ...
            if ($refundData['refund_price'] > 0) {
                switch ($order['pay_type']) {
                    ...
                    case PayServices::ALIAPY_PAY:
                        //支付宝退款
                        AliPayService::instance()->refund($order['order_id'], $refundData['pay_price']);
                        break;
                }
            }
            ...
            $userBillServices->income('pay_product_refund', $order['uid'], ['number' => $refundData['refund_price'], 'payType' => PayServices::PAY_TYPE[$order['pay_type']] ?? '未知'], $usermoney, $order['id']);
            //修改开票数据退款状态
