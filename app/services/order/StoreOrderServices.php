...
class StoreOrderServices extends BaseServices
{
    ...
    public function getOrderData(int $uid = 0)
    {
        ...
        $data['yue_pay_status'] = (int)sys_config('balance_func_status') && (int)sys_config('yue_pay_status') == 1 ? (int)1 : (int)2;//货币支付 1 开启 2 关闭
...
//        }
    ...
    public function growth($nowValue, $lastValue)
    {
        ...
        if ($lastValue == 0) return round(bcdiv($nowValue, 100, 4), 2);
        if ($nowValue == 0) return -round(bcdiv($lastValue, 100, 4), 2);
    ...
    public function homeStatics()
    {
        ...
        $info = array_values(compact('visits',  'user'));
        ...
        $info[0]['total_name'] = '本月访问量';
        //$info[2]['total_name'] = '本月订单量';
        $info[1]['total_name'] = '本月新增用户';
        return $info;
    }
    ...
    public function getOrderConfirmData(array $user, $cartId, bool $new)
    {
        ...
        $priceGroup = $computedServices->getOrderPriceGroup($cartInfo, $addr, $user);
        $other = [
    ...
    public function checkPaytype(string $payType)
    {
        ...
        switch ($payType) {
            ...
            case 'offline':
                ...
                break;
            case PayServices::ALIAPY_PAY:
                $res = sys_config('ali_pay_status') == 1 ? true : false;
                break;
        }
        return $res;
    }
    ...
    {
        return $this->dao->getOrderListByWhere($where, $field);
    }

    /** 需要批量删除的订单加入redis集合及数据队列
     * @param array $where
     * @param string $field
     * @param array $pid
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function orderDelAddRedis(array $where = [], $field = "*", array $oid = [], string $redisKey, $type)
    {
        ...
        return $queueService->getQueueRedisData($redisKey, $type, $oid, $where, $field);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function adminDelOrder($type)
    {
        ...
        $queueService->addQueueDoing("", $queueType['id']);
        $redisKey = $queueType['execute_key'];
        $pids = $redis->sMembers($redisKey);
        $chunkPids = array_chunk($pids, 1000, true);
        foreach ($chunkPids as $v) {
            $this->orderDel($v, $redisKey, $queueType);
        }
        //发完后将队列置为完成
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function orderBatchCache(array $expreData, $queueType, $cacheType, $queueName, $source)
    {
        if (!$expreData) throw new AdminException('缺少订单数据');
        /** @var QueueAuxiliaryServices $auxiliaryService */
        $auxiliaryService = app()->make(QueueAuxiliaryServices::class);
        return $auxiliaryService->saveOrderCacheLog($expreData, $queueName, $queueType, $cacheType, $source);
    }
    ...
    public function adminQueueOrderDo(array $data, bool $is_again = false)
    {
        ...
        $queueWhere['status'] = [0, 3];
        if (isset($data['queueId']) && $data['queueId']) $queueWhere['id'] = $data['queueId'];
        $queueType = $queueService->getQueueOne($queueWhere);
        $ids = $auxiliaryService->getCacheOidList($queueType['id'], $data['cacheType']);
        $data['ids'] = array_column($ids, 'relation_id');
        $data['queueId'] = $queueType['id'];
        //if ($queueType['status'] == 2) throw new ValidateException('任务已完成');
        //把队列需要执行的入参数据存起来，以便队列执行失败后接着执行，同时队列状态改为正在执行状态。
        $queueService->addQueueDoing($data, $queueType['id'], $is_again);
        $oids = $auxiliaryService->getOrderExpreList(['binding_id' => $queueType['id'], 'type' => $data['cacheType'], 'status' => [0, 2]]);
        $oids = $oids ? array_column($oids, 'relation_id') : [];
        // $chunkPids = array_chunk($oids, 1000, true);
        $data['queueId'] = $queueType['id'];
        foreach ($oids as $v) {
            /** @var BatchHandleJob $sss */
            // $sss= app()->make(BatchHandleJob::class);
            // $a = $sss->doJob($v, $data['queueType'], $data);
            Queue::instance()->job(\crmeb\jobs\BatchHandleJob::class)->data($v, $data['queueType'], $data)->setQueueName("BATCH-CRMEB")->push();
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getOrderDumpData(array $where, $field = "*", array $oid = [], $queueType, $cacheType)
    {
        if (!$queueType) throw new ValidateException('缺少队列执行类型');
        /** @var QueueAuxiliaryServices $auxiliaryService */
        $auxiliaryService = app()->make(QueueAuxiliaryServices::class);
        /** @var QueueServices $queueService */
        $queueService = app()->make(QueueServices::class);
        $queueType = $queueService->getQueueOne(['type' => $queueType, 'status' => [0, 1, 3]]);
        if ($queueType) {
            $log = $auxiliaryService->getOrderCacheOne(['binding_id' => $queueType['id'], 'type' => $cacheType, 'status' => [0, 2]]);
            if (!$log) throw new AdminException('任务数据异常，前往任务列表清除异常数据');
            if ($log && $queueType['status'] == 0) throw new AdminException('上次批量任务尚未执行，请前往任务列表手动执行');
            if ($log && $queueType['status'] == 3) throw new AdminException('上次批量任务执行失败，请前往任务列表手动执行');
            if ($log && $queueType['status'] == 1) throw new AdminException('有正在执行中的任务，请耐心等待，若长时间无反应，前往任务列表修复异常数据，再手动执行');
        }
        if (!$oid) {
            $oid = $this->dao->getOrderDumpData($where, $field);
            $oid = array_column($oid, 'id');
        }
        //根据短信条数和电子面单条数判断最多能批量执行的任务条数
        /** @var ServeServices $serverService */
        $serverService = app()->make(ServeServices::class);
        $serveInfo = $serverService->user()->getUser();
        $canNum = 0;
        /*  if ($serveInfo) {
              $dumpNum = $serveInfo['dump']['num'];
              $smsNum = $serveInfo['sms']['num'];
              $canNum = ($dumpNum < $smsNum) ? $dumpNum : $smsNum;
          }
          if ($canNum == 0) throw new AdminException('您的可使用短信条数或电子面单条数不足，请前往一号通购买');
          if ($canNum < count($oid)) throw new AdminException('您的可使用短信条数或电子面单条数不足，最多只能批量执行' . $canNum . '条数据' . '请上传或者选择符合数量数据进行操作');*/
        return $oid;
    }
    ...
    {
        return $this->dao->getCanDevlieryOrder($key, $value);
    }
