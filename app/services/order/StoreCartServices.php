...
class StoreCartServices extends BaseServices
{
    ...
    public function getUserProductCartListV1($uid, $cartIds = '', bool $new, $status = 0)
    {
        if ($new) {
            ...
            if ($uid) {
                ...
                $userLevel = $user->value(['uid' => $uid], 'level');
                $discount = $systemLevel->value(['id' => $userLevel], 'discount');
            } else {
                $discount = null;
            }
            ...
            foreach ($cartInfo as &$item) {
                ...
                if (isset($productInfo['attrInfo']['product_id']) && $item['product_attr_unique']) {
                    ...
                    if ($item['seckill_id'] == 0 && $item['bargain_id'] == 0 && $item['combination_id'] == 0) {
                        //$item['truePrice'] = $productServices->setLevelPrice($productInfo['attrInfo']['price'] ?? 0, $uid, $userInfo, $vipStatus, true, $discount, $item['productInfo']['attrInfo']['vip_price'], $productInfo['is_vip'] ?? 0, true);
                        $item['vip_truePrice'] = (float)$productServices->setLevelPrice($productInfo['attrInfo']['price'], $uid, $userInfo, $vipStatus, false, $discount, $item['productInfo']['attrInfo']['vip_price'], $productInfo['is_vip'] ?? 0, true);
                    } else {
                    ...
                    if ($item['seckill_id'] == 0 && $item['bargain_id'] == 0 && $item['combination_id'] == 0) {
                        //$item['truePrice'] = $productServices->setLevelPrice($item['productInfo']['price'] ?? 0, $uid, $userInfo, $vipStatus, true, $discount, $item['productInfo']['attrInfo']['vip_price'] ?? 0, $item['productInfo']['is_vip'] ?? 0, true);
                        $item['vip_truePrice'] = (float)$productServices->setLevelPrice($item['productInfo']['price'], $uid, $userInfo, $vipStatus, false, $discount, $item['productInfo']['attrInfo']['vip_price'] ?? 0, $item['productInfo']['is_vip'] ?? 0, true);
                    } else {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function checkProductStock(int $uid, int $cartNum, string $unique, $productId, int $seckillId, int $bargainId, int $combinationId, bool $new)
    {
        ...
        if ($seckillId) {
            ...
            //检查商品活动状态
            $seckillService->getSeckillCount($seckillId);
     ...
     * @param string $orderKey 订单类型
     * @param bool $orderVal 订单数值
     * @param int $isHead 是否团长
     * @param int $isPromoter 是否群主
     * @return mixed|string
     * @throws \Psr\SimpleCache\InvalidArgumentException
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\DbException
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function setCart(int $uid, int $product_id, int $cart_num = 1, string $product_attr_unique = '', string $type = 'product', bool $new = false, int $combination_id = 0, int $seckill_id = 0, int $bargain_id = 0, string $orderKey = 'integral', $orderVal = 0, int $isHead = 0, int $isPromoter = 0)
    {
        /** @var StoreProductServices $productServices */
        $productServices = app()->make(StoreProductServices::class);
        /** @var StoreSeckillServices $seckillService */
        ...
        /** @var StoreCombinationServices $combinationService */
        $combinationService = app()->make(StoreCombinationServices::class);
        [$res, $product_attr_unique, $bargainPriceMin, $cart_num] = $this->checkProductStock($uid, $cart_num, $product_attr_unique, (int)$product_id, $seckill_id, $bargain_id, $combination_id, $new);
        ...
        if ($new) { //TODO 直接购买
            ...
            $info['orderKey'] = $orderKey;
            $info['orderVal'] = $orderVal;
            $info['isHead'] = $isHead;
            $info['isPromoter'] = $isPromoter;
            $info['isSpell'] = $isSpell;
            $info['truePrice'] = $orderVal ?? 0;
            ...
            if (!$bargain_id || !$combination_id || !$seckill_id) {
                ...
                $user = app()->make(UserServices::class);
                $userInfo = $user->getUserInfo($uid);
                //$info['truePrice'] = $productServices->setLevelPrice($info['productInfo']['attrInfo']['price'], $uid, $userInfo, $vipStatus, true, 0, $info['productInfo']['attrInfo']['vip_price'] ?? 0, $info['productInfo']['is_vip'] ?? 0, true);
                $info['vip_truePrice'] = (float)$productServices->setLevelPrice($info['productInfo']['attrInfo']['price'], $uid, $userInfo, $vipStatus, false, 0, $info['productInfo']['attrInfo']['vip_price'] ?? 0, $info['productInfo']['is_vip'] ?? 0, true);
            } else {
            ...
            $cart = $this->dao->getOne(['type' => $type, 'orderKey' => $orderKey, 'orderVal' => $orderVal, 'uid' => $uid, 'product_id' => $product_id, 'product_attr_unique' => $product_attr_unique, 'is_del' => 0, 'is_new' => 0, 'is_pay' => 0, 'status' => 1]);
            if ($cart) {
                ...
                $add_time = time();
                return $this->dao->save(compact('uid', 'product_id', 'cart_num', 'product_attr_unique', 'type', 'orderKey','orderVal', 'isHead', 'isPromoter', 'isSpell', 'add_time'))->id;
            }
        }
    }
    ...
    public function getUserCartList(int $uid, int $status, string $cartIds = '')
    {
        ...
        /** @var SystemUserLevelServices $systemLevel */
        $systemLevel = app()->make(SystemUserLevelServices::class);
        $discount = $systemLevel->value(['id' => $userInfo['level']], 'discount');
        foreach ($list as &$item) {
            ...
            if (isset($productInfo['attrInfo']['product_id']) && $item['product_attr_unique']) {
                ...
                $item['vip_truePrice'] = (float)$productServices->setLevelPrice($productInfo['attrInfo']['price'] ?? 0, $uid, $userInfo, $vipStatus, false, $discount, $item['attrInfo']['vip_price'] ?? 0, $productInfo['is_vip'] ?? 0, true);
            } else {
                $item['costPrice'] = $item['productInfo']['cost'] ?? 0;
                $item['trueStock'] = $item['productInfo']['stock'] ?? 0;
