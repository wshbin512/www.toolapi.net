...
class StoreOrderTakeServices extends BaseServices
{
    ...
    public function sendBackOrderBrokerage($orderInfo, $spread_uid, $brokeragePrice)
    {
        ...
        if (strtolower($userType) == 'routine') {
            ...
            if ($openid) Queue::instance()->do('sendOrderBrokerageSuccess')->job(RoutineTemplateJob::class)->data($openid, $brokeragePrice, $goodsName)->push();
        } else {
            $openid = $wechatServices->uidToOpenid($spread_uid, 'wechat');
            if ($openid) Queue::instance()->do('sendOrderBrokerageSuccess')->job(TemplateJob::class)->data($openid, $brokeragePrice, $goodsName, $goodsPrice, $orderInfo['add_time'])->push();
        }
    }
