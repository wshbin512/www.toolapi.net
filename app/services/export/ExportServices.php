...
class ExportServices extends BaseServices
{
    ...
    public function export($header, $title_arr, $export = [], $filename = '', $suffix = 'xlsx', $is_save = false)
    {
        ...
        if ($exportNum > 6000) throw new AdminException('单次导出数据最大不得超过6000条，请分条件分批次进行导出');
     ...
     * @param $data 导出数据
     */
    public function storeOrder($data = [], $type = "")
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $item) {
...
HTML;
        ...
        if (!$type) {
            ...
            $header = ['订单号', '物流公司', '物流编码', '物流单号', '发货地址', '用户信息', '手机号', '商品信息'];
     ...
     * @return mixed
     */
    public function batchOrderDelivery($data = [], $queueType)
    {
        $export = [];
        if (!empty($data)) {
            foreach ($data as $index => $item) {
