...
class AgentManageServices extends BaseServices
{
    ...
    public function lookXcxCode(int $uid)
    {
        ...
        if (!$imageInfo) {
            /** @var QrcodeServices $qrcode */
            $qrcode = app()->make(QrcodeServices::class);
            $resForever = $qrcode->qrCodeForever($uid, 'spread');
            if ($resForever) {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function lookH5Code(int $uid, string $appId)
    {
        $userInfo = app()->make(UserServices::class)->getUserInfo($uid);
        if (!$userInfo) {
            throw new AdminException('数据不存在');
        ...
        $imageInfo = $systemAttachmentModel->getInfo(['name' => $name]);
        if (!$imageInfo) {
            $urlCode = QrcodeService::getWechatQrcodePath($uid . '_h5_' . $userInfo['is_promoter'] . '_user.jpg', '/pages/users/download/index?spread=' . $uid . '&appId=' . $appId);
        } else {
            $urlCode = sys_config('site_url').$imageInfo['att_dir'];
        }
        return ['code_src' => $urlCode];
    }
