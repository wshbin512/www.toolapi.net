...
class WechatUserServices extends BaseServices
{
     ...
     * @param $spid
     */
    public function saveUser($openid, $spid = 0)
    {
        if ($this->getWechatUserInfo(['openid' => $openid]))
            $this->updateUser($openid, $spid);
        else
            $this->setNewUser($openid, $spid);
    }
     ...
     * @return bool
     */
    public function updateUser($openid, $spid = 0)
    {
        $userInfo = WechatService::getUserInfo($openid);
        $userInfo['tagid_list'] = implode(',', $userInfo['tagid_list']);
        try {
            if (!$this->dao->update($openid, $userInfo->toArray(), 'openid')) throw new AdminException('更新失败');
            $uid = $this->dao->value(['openid' => $openid], 'uid');
            /** @var UserServices $userService */
            $userService = app()->make(UserServices::class);
            $user = $userService->get($uid);
            if ($user) {
                // 如果没有上级并且分销关系绑定为所有用户的时候，绑定上下级关系
                if (!$user->spread_uid && sys_config('brokerage_bindind') == 1) {
                    $user->spread_uid = $spid;
                    $user->spread_time = time();
                    if (!$user->save()) throw new AdminException('更新失败');
                }
            }
        } catch (\Throwable $e) {
        }
     ...
     * @return object
     */
    public function setNewUser($openid, $spid = 0)
    {
        $userInfo = WechatService::getUserInfo($openid);
        ...
        if (!$wechatInfo) {
            ...
            $userInfoData = $userServices->setUserInfo($userInfo, $spid);
            if (!$userInfoData) {
    ...
    public function wechatOauthAfter(array $data)
    {
        ...
        /** @var UserServices $userServices */
        $userServices = app()->make(UserServices::class);
        if ($spreadId && !$userServices->getUserInfo((int)$spreadId)) {
            $spreadId = 0;
        }
