...
class SystemStoreStaffServices extends BaseServices
{
    ...
    {
        if ($formData) {
            $field[] = $this->builder->frameImage('image', '更换头像', $this->url('admin/widget.images/index', array('fodder' => 'image'),true), $formData['avatar'] ?? '')->icon('ios-add')->width('950px')->height('420px');
        } else {
        ...
        $field[] = $this->builder->input('staff_name', '核销员名称', $formData['staff_name'] ?? '')->col(24);
        $field[] = $this->builder->input('phone', '手机号码', $formData['phone'] ?? '')->col(24);
