...
class ProductStatisticServices extends BaseServices
{
    ...
    public function basicInfo($where, $time)
    {
        ...
        $data['order'] = $storeOrder->sum($where, 'total_num', true);//下单件数
        ...
        $data['refundPrice'] = $userBill->getBillSum($where + ['type' => 'pay_product_refund']);//退款金额
        $data['refund'] = $userBill->getTotalSum($where + ['type' => 'pay_product_refund']);//退款件数
    ...
    public function trend($time, $num, $excel = false)
    {
        ...
        $browse = array_column($storeVisit->getProductTrend($time, $timeType, 'sum(count)'), 'num', 'days');
        $user = array_column($storeVisit->getProductTrend($time, $timeType, 'count(distinct(uid))'), 'num', 'days');
        $pay = array_column($storeOrder->getProductTrend($time, $timeType, 'pay_time', 'sum(pay_price)'), 'num', 'days');
        $refundList = $userBillServices->getUserRefundPriceList($time, $timeType, 'sum(number)', 'add_time');
