...
use app\services\activity\{StoreBargainServices, StoreCombinationServices, StorePinkServices, StoreSeckillServices};
...
class StoreOrderController
{
    ...
    public function create(Request $request, StoreBargainServices $bargainServices, StorePinkServices $pinkServices, StoreOrderCreateServices $createServices, StoreSeckillServices $seckillServices, UserInvoiceServices $userInvoiceServices, StoreOrderInvoiceServices $storeOrderInvoiceServices, StoreCombinationServices $combinationServices, $key)
    {
        ...
        [$addressId, $couponId, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, $from, $shipping_type, $real_name, $phone, $storeId, $news, $invoice_id, $quitUrl, $orderKey, $orderVal, $isHead, $isPromoter] = $request->postMore([
            [['addressId', 'd'], 0],
            [['couponId', 'd'], 0],
            ['payType', ''],
            ['useIntegral', 0],
        ...
        if ($pinkId) {
            ...
            if ($pinkServices->isPink($pinkId, $uid)){
                // TODO 修改完订单，准备重新支付
                $spell = $pinkServices->isPinkSpell($pinkId, $uid, $orderVal);
                if($spell == false){
                    return app('json')->status('ORDER_EXIST', '订单生成失败，你已经在该团内不能再参加了', ['orderId' => $this->services->getStoreIdPink($pinkId, $uid)]);
                } else {
                    return app('json')->status('success', '支付成功', ['orderId' => $this->services->getStoreIdPink($pinkId, $uid)]);
                }
            }
        ...
        if ($seckill_id || $combinationId) {
            ...
            foreach ($cartInfo as $item) {
                ...
                if ($item['combination_id']) {
                    $combinationServices->checkoutProductStock($uid, (int)$item['combination_id'], (int)$item['cart_num'], $item['product_attr_unique'] ?? '');
                }
            }
        }

        try {
            $order = $createServices->createOrder($uid, $key, $cartGroup, $request->user()->toArray(), $addressId, $payType, !!$useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, $isChannel, $shipping_type, $real_name, $phone, $storeId, !!$news, $orderKey, $orderVal, $isHead, $isPromoter);
        } catch (\Throwable $e) {
            //生成失败归还库存
            if ($seckill_id) $seckillServices->orderFalseStock($cartInfo);
            return app('json')->fail($e->getMessage());
        }
        if ($order === false) {
            //回退占用
            $seckillServices->cancelOccupySeckillStock($cartGroup['cartInfo'], $key);
            $seckillServices->rollBackStock($cartInfo);
        ...
        if ($orderId) {
            //$this->paySelect($request,$payType,$orderInfo,$uid,$info,$from,$quitUrl);
            switch ($payType) {
                ...
                case PayServices::YUE_PAY:
                    ...
                    if ($pay['status'] === true)
                        return app('json')->status('success', '货币支付成功', $info);
                    else {
                        if (is_array($pay))
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function paySelect($request,$payType,$orderInfo,$uid,$info,$from,$quitUrl){
        switch ($payType) {
            case PayServices::WEIXIN_PAY:
                if ($orderInfo['paid']) return app('json')->fail('支付已支付!');
                //支付金额为0
    ...
    public function again(Request $request, StoreCartServices $services)
    {
        ...
        foreach ($order['cartInfo'] as $v) {
            ...
            else $cateId[] = $services->setCart($request->uid(), (int)$v['product_id'], (int)$v['cart_num'], isset($v['productInfo']['attrInfo']['unique']) ? $v['productInfo']['attrInfo']['unique'] : '', 'product', true);
        }
    ...
    public function pay(Request $request, StorePinkServices $services, OrderPayServices $payServices, YuePayServices $yuePayServices)
    {
        ...
        switch ($order['pay_type']) {
            ...
            case PayServices::YUE_PAY:
                ...
                if ($pay['status'] === true)
                    return app('json')->status('success', '货币支付成功');
                else {
                    if (is_array($pay))
    ...
    public function detail(Request $request, StoreOrderEconomizeServices $services, $uni)
    {
        ...
        $order['yue_pay_status'] = (int)sys_config('balance_func_status') && (int)sys_config('yue_pay_status') == 1 ? (int)1 : (int)2;//货币支付 1 开启 2 关闭
        ...
        //核算优惠金额
        $vipTruePrice = array_column($orderData['cartInfo'], 'vip_sum_truePrice');
